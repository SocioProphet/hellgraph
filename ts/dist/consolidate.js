...
function consolidate() {
    ...
    for (const atom of g.allNodes()) {
        if (!atom.labels.includes('FeatureAtom'))
            continue;
        if ((0, ecan_1.getVLTI)(atom.id))
            continue; // VLTI atoms are exempt
    ...
    // ── 3. Deep PLN pass ──────────────────────────────────────────────────────
    const plnResult = (0, pln_1.forwardChain)({ maxIters: 200, runRevision: true, runAbduction: true });
    // ── 4. VLTI promotion ─────────────────────────────────────────────────────
    let vltiPromoted = 0;
    for (const atom of g.allNodes()) {
        if (!atom.labels.includes('FeatureAtom'))
        ...
        if ((0, ecan_1.getVLTI)(atom.id))
            continue;
        if ((0, ecan_1.getLTI)(atom.id) >= LTI_VLTI_THRESHOLD) {
    ...
    (0, prometheus_1.recordAttentionSnapshot)();
    (0, prometheus_1.runSINDyPass)().then(candidate => {
        if (candidate) {
