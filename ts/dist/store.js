...
exports.HellGraphStore = void 0;
exports.getHellGraph = getHellGraph;
...
function encodeValue(v) {
    ...
    if (typeof v === 'boolean')
        return { kind: 'string', value: [v ? 'true' : 'false'] };
...
class HellGraphStore {
    ...
    projectEdge(evalAtom) {
        ...
        if (!pred?.name || !from?.name || !to?.name)
            return null;
        const properties = {};
    ...
    addNode(id, labels, properties = {}) {
        ...
        for (const [k, v] of Object.entries(properties)) {
            ...
            if (enc)
                this.as.setValue(atom.handle, PROP_PREFIX + k, enc);
        }
        return this.projectNode(this.as.getAtom(atom.handle));
    }
    addEdge(label, from, to, properties = {}) {
        ...
        for (const [k, v] of Object.entries(properties)) {
            const enc = encodeValue(v);
            if (enc)
                this.as.setValue(evalA.handle, PROP_PREFIX + k, enc);
        }
    ...
    setNodeProperty(id, key, value) {
        ...
        if (enc)
            this.as.setValue((0, atomspace_1.nodeHandle)(ENTITY, id), PROP_PREFIX + key, enc);
    }
    // ─── Read path ────────────────────────────────────────────────────────────
    getNode(id) {
    ...
    outEdges(nodeId, label) {
        const conceptH = (0, atomspace_1.nodeHandle)(ENTITY, nodeId);
        return this.adjacentEdges(conceptH, 0, label);
    }
    inEdges(nodeId, label) {
        const conceptH = (0, atomspace_1.nodeHandle)(ENTITY, nodeId);
    ...
    adjacentEdges(conceptH, pos, label) {
        const out = [];
        for (const listLink of this.as.getIncoming(conceptH, 'ListLink')) {
            if (listLink.outgoing?.[pos] !== conceptH)
                continue;
    ...
    // ─── Stats / health ──────────────────────────────────────────────────────────
    get logicalClock() { return this.as.logicalClock; }
    get id() { return this.as.id; }
    nodeCount() { return this.as.getByType(ENTITY, false).length; }
    edgeCount() { return this.as.getByType('EvaluationLink', false).length; }
    orphanNodeCount() {
        let n = 0;
        for (const atom of this.as.getByType(ENTITY, false)) {
    ...
    danglingEdgeCount() { return this.as.danglingLinkCount(); }
    logTail(n = 20) {
        return this.as.logTail(n).map((e) => ({ seq: e.seq, ts: e.ts, op: e.op, payload: e.payload }));
    }
    earliestTs() { return this.as.earliestTs(); }
    latestTs() { return this.as.latestTs(); }
    /** Escape hatch to the underlying metagraph for hypergraph-native operations. */
    atomspace() { return this.as; }
}
exports.HellGraphStore = HellGraphStore;
function getHellGraph() {
    if (!globalThis.__hellgraph_store__) {
        globalThis.__hellgraph_store__ = new HellGraphStore((0, atomspace_1.getAtomSpace)());
    }
