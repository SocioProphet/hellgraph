...
class ShapeIndex {
    ...
    add(s, p, o) {
        ...
        sp.get(p).push(o);
        // pos
        if (!this.pos_idx.has(p))
            this.pos_idx.set(p, new Map());
    ...
    get(s, p) { return this.spo.get(s)?.get(p) ?? []; }
    one(s, p) { return this.get(s, p)[0]; }
    subjects(p, o) { return this.pos_idx.get(p)?.get(o) ?? []; }
    allValues(p) {
...
function buildIndex(triples) {
    ...
    for (const t of triples)
        idx.add(termStr(t.s), t.p.value, termStr(t.o));
    return idx;
}
function collectList(idx, head) {
    const items = [];
    let cur = head;
    ...
    return Array.from(ids).map((id) => {
        ...
        for (const bnode of idx.get(id, SH + 'property')) {
            ...
            if (!path)
                continue;
            const ps = { path };
            const mc = idx.one(bnode, SH + 'minCount');
            if (mc !== undefined)
                ps.minCount = parseInt(mc, 10);
            const xc = idx.one(bnode, SH + 'maxCount');
            if (xc !== undefined)
                ps.maxCount = parseInt(xc, 10);
            const dt = idx.one(bnode, SH + 'datatype');
            if (dt)
                ps.datatype = dt;
            const nk = idx.one(bnode, SH + 'nodeKind');
            if (nk)
                ps.nodeKind = nk;
            const pat = idx.one(bnode, SH + 'pattern');
            if (pat)
                ps.pattern = pat;
            ps.patternFlags = idx.one(bnode, SH + 'flags');
            const cls = idx.one(bnode, SH + 'class');
            if (cls)
                ps.cls = cls;
            const inHead = idx.one(bnode, SH + 'in');
            if (inHead)
                ps.in = collectList(idx, inHead);
            const hv = idx.one(bnode, SH + 'hasValue');
            if (hv !== undefined)
                ps.hasValue = hv;
            const mi = idx.one(bnode, SH + 'minInclusive');
            if (mi !== undefined)
                ps.minInclusive = parseFloat(mi);
            const xi = idx.one(bnode, SH + 'maxInclusive');
            if (xi !== undefined)
                ps.maxInclusive = parseFloat(xi);
            const ml = idx.one(bnode, SH + 'minLength');
            if (ml !== undefined)
                ps.minLength = parseInt(ml, 10);
            ps.message = idx.one(bnode, SH + 'message');
            properties.push(ps);
        }
        const sparqlConstraints = [];
        for (const bnode of idx.get(id, SH + 'sparql')) {
            const select = idx.one(bnode, SH + 'select');
            if (select)
                sparqlConstraints.push({ select, message: idx.one(bnode, SH + 'message') });
        }
        const sparqlRules = [];
        for (const bnode of idx.get(id, SH + 'rule')) {
            const construct = idx.one(bnode, SH + 'construct');
            if (construct)
                sparqlRules.push({ construct });
        }
        return {
            id,
...
function getTargetNodes(shape, triples) {
    const nodes = new Set();
    ...
    if (shape.targetSubjectsOf) {
        ...
        for (const t of triples) {
            if (t.predicate === prop || t.predicate === propShort)
                nodes.add(t.subject);
        }
    }
    return Array.from(nodes);
}
function getValues(focusNode, path, triples) {
...
// ─── Constraint checkers ─────────────────────────────────────────────────────
function inferXsdDatatype(value) {
...
function checkPropertyShape(focusNode, ps, triples, shapeId) {
    const vals = getValues(focusNode, ps.path, triples);
    const base = { focusNode, path: ps.path, severity: 'Violation', shape: shapeId };
    const msg = (m) => ps.message ?? m;
    const violations = [];
 ...
 * CONSTRUCT queries.
 */
function validateGraph(store, shapesText) {
    const idx = buildIndex((0, turtle_1.parseTurtle)(shapesText));
    ...
    for (const shape of shapes) {
        ...
        for (const focusNode of targets) {
            ...
            for (const sc of shape.sparqlConstraints) {
                try {
                    const result = (0, sparql_1.runSparql)(store, sc.select);
        ...
        for (const rule of shape.sparqlRules) {
            try {
                const constructed = (0, sparql_1.runSparqlConstruct)(store, rule.construct);
                // Find sh:result subjects — each is a ValidationResult bnode
                const resultObjs = new Set(constructed
 ...
 * triples written.
 */
function applyRules(store, shapesText) {
    const idx = buildIndex((0, turtle_1.parseTurtle)(shapesText));
    const shapes = extractShapes(idx);
    let added = 0;
    for (const shape of shapes) {
        for (const rule of shape.sparqlRules) {
            try {
                const newTriples = (0, sparql_1.runSparqlConstruct)(store, rule.construct);
                // Skip rules that produce validation result atoms (those are for validateGraph)
                const isValidationRule = newTriples.some(t => t.predicate === SH + 'result' || t.predicate === 'sh:result');
                if (isValidationRule)
                    continue;
