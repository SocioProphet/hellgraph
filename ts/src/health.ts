...
export function computeGraphHealth(): GraphHealthStatus {
  ...
  let status: GraphHealthStatus['status'] = 'healthy'
  if (g.nodeCount() === 0) status = 'unknown'
  else if (dangling > 0) status = 'degraded'
  else if (orphans > g.nodeCount() * 0.5) status = 'degraded'
...
export function computeTimeService(): TimeServiceStatus {
  const g = getHellGraph()
