...
export class StorageNodeClient {
  ...
  async stats(): Promise<StorageNodeStats> {
    const res = await fetch(`${this.baseUrl}/api/atomspace/stats`, { headers: this.headers })
    if (!res.ok) throw new Error(`StorageNode stats failed: ${res.status}`)
    return res.json() as Promise<StorageNodeStats>
  }

  async fetchAtom(handle: Handle): Promise<Atom | null> {
    const res = await fetch(
      `${this.baseUrl}/api/atomspace/atom/${encodeURIComponent(handle)}`,
      { headers: this.headers, signal: AbortSignal.timeout(5000) },
    )
    if (res.status === 404) return null
    if (!res.ok) throw new Error(`fetchAtom failed: ${res.status}`)
    return res.json() as Promise<Atom>
  }

  async fetchBatch(handles: Handle[]): Promise<Atom[]> {
    ...
    if (!res.ok) throw new Error(`fetchBatch failed: ${res.status}`)
    const data = await res.json() as { atoms: Atom[] }
    return data.atoms
  }

  async fetchByType(type: string): Promise<Atom[]> {
    const res = await fetch(
      `${this.baseUrl}/api/atomspace/by-type/${encodeURIComponent(type)}`,
      { headers: this.headers, signal: AbortSignal.timeout(10_000) },
    )
    if (!res.ok) throw new Error(`fetchByType failed: ${res.status}`)
    const data = await res.json() as { atoms: Atom[] }
    return data.atoms
  }
  ...
  async bulkSync(atoms: Atom[], sourceId?: string): Promise<number> {
    if (atoms.length === 0) return 0
    const entries: AtomLogEntry[] = atoms.map((a) => ({
      seq: a.createdAtSeq,
      ts: a.createdAt,
    ...
    for (let i = 0; i < entries.length; i += 500) {
      ...
      if (!res.ok) throw new Error(`bulkSync batch failed: ${res.status}`)
   ...
   */
  subscribeChangeFeed(
    onEntry: (entry: AtomLogEntry) => void,
      ...
      try {
        ...
        while (true) {
          ...
          for (const line of lines) {
            ...
            if (!raw || raw.startsWith('{"type":"connected"}')) continue
            try {
              const event = JSON.parse(raw) as { entry?: AtomLogEntry; op?: string }
              if (event.entry) onEntry(event.entry)
